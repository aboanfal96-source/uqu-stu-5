...
/* ============================================================
   دوال استخراج النص من رد الـ AI
   ============================================================ */
function sec(t,m){
  if(!t)return'—';
  // جرّب الصيغ المختلفة التي قد يردّ بها النموذج
  var variants=[m, m.replace(':\n',': '), m.replace(':\n',':')];
  var i=-1, used=m;
  for(var v=0;v<variants.length;v++){
    i=t.indexOf(variants[v]);
    if(i!==-1){used=variants[v];break;}
  }
  if(i===-1)return'—';
...
function showPg(id){
  PAGES.forEach(function(p){document.getElementById(p).classList.remove("on");});
  document.getElementById(id).classList.add("on");
  var r=document.getElementById("hRole");
  if(id==="pgA"){r.textContent="🔐 مدير القسم";r.className="hbtn hbtn-a";}
  else if(id==="pgEF"){r.textContent="👷 بوابة الموظفين";r.className="hbtn hbtn-e";}
  else{r.textContent="نظام الدراسات الخمسة";r.className="hbtn hbtn-e";}
}
...
/* ============================================================
   قاعدة البيانات (JSONBin)
   ============================================================ */
async function readDB(){
  var r=await fetch("https://api.jsonbin.io/v3/b/"+BIN_ID+"/latest",{headers:{"X-Master-Key":BIN_KEY}});
  if(!r.ok)throw new Error("DB error "+r.status);
  var d=await r.json();
  return(d.record&&d.record.requests)?d.record.requests:[];
}
async function writeDB(list){
  var r=await fetch("https://api.jsonbin.io/v3/b/"+BIN_ID,{method:"PUT",headers:{"Content-Type":"application/json","X-Master-Key":BIN_KEY},body:JSON.stringify({requests:list.slice(0,200)})});
  if(!r.ok)throw new Error("Save error "+r.status);
}

/* ============================================================
   استدعاء Anthropic API عبر Serverless proxy
   ============================================================ */
async function callAI(system, userMsg){
  var r=await fetch("/api/claude",{
    method:"POST",
    headers:{"Content-Type":"application/json"},
    body:JSON.stringify({system:system, max_tokens:4000, messages:[{role:"user",content:userMsg}]})
  });
  var d=await r.json();
...
ge("btnNW").addEventListener("click",function(){
  ge("okBox").style.display="none";
  ge("fInner").style.display="block";
  document.querySelectorAll("#fInner input,#fInner textarea,#fInner select").forEach(function(e){e.value="";});
  ge("sbBtn").disabled=false;
  ge("sbBtn").innerHTML="📨 إرسال طلب الدراسة";
});
...
function doLogin(){
  var pw=gv("pw");
  ge("lerr").textContent="";
  if(!pw){ge("lerr").textContent="أدخل كلمة المرور";return;}
  if(pw!==ADMIN_PW){ge("lerr").textContent="كلمة المرور غير صحيحة";return;}
  showPg("pgA");
  loadAdmin();
}
...
async function submitReq(){
  ...
  try{
    var list=await readDB();
    var id="REQ-"+Date.now();
    list.unshift({id:id,status:"pending",submittedAt:new Date().toISOString(),requesterName:rn,department:dp,phone:gv("ph"),email:gv("em"),projectName:pn,projectType:pt,location:gv("lo"),costConstruction:co,duration:du,fiscalYear:gv("fy")||"1446-1447 هـ",beneficiary:gv("be")||"منسوبو الجامعة",description:de,justification:ju,notes:gv("no")});
    await writeDB(list);
    ge("fInner").style.display="none";
    ge("okBox").style.display="block";
    ge("okId").textContent="رقم الطلب: "+id;
...
async function loadAdmin(){
  try{var list=await readDB();renderAdmin(list);}
  catch(e){alert("خطأ: "+e.message);}
}

function swTab(n){
  [1,2,3].forEach(function(i){ge("t"+i).classList.toggle("on",i===n);});
  ge("tp").style.display=n===1?"flex":"none";
  ge("td").style.display=n===2?"flex":"none";
  ge("tl").style.display=n===3?"block":"none";
  ge("tp").style.flexDirection="column";
  ge("td").style.flexDirection="column";
}
...
function renderAdmin(reqs){
  ...
  var gen=reqs.filter(function(r){return r.status==="generated";});
  ...
  var logAll=gen.map(function(r){return{date:r.reviewedAt,name:r.projectName,who:r.requesterName,dept:r.department,c:"#0d7a7a",ic:"✅ تم التوليد"};})
    .concat(rej.map(function(r){return{date:r.reviewedAt,name:r.projectName,who:r.requesterName,dept:r.department,c:"#c03030",ic:"❌ مرفوض"};}))
...
async function doAct(id,action){
  ...
  var btn=document.querySelector("#c_"+id+" .bok");
  ...
  try{
    ...
    if(action==="rejected"){
      item.status="rejected";list[idx]=item;await writeDB(list);ov(false);loadAdmin();return;
    }
    // حساب التكاليف
    var cn=parseFloat(String(item.costConstruction||"0").replace(/,/g,""))||0;
    var sn=Math.round(cn*0.10),tot=cn+sn,opx=Math.round(tot*0.00025*30),grd=tot+opx,dn=parseInt(item.duration)||12;
    ...
    var SYS="أنت خبير استشاري لدراسات الجدوى. قاعدة واحدة صارمة: ابدأ ردك مباشرة بالعلامة الأولى (مثل VISION:) بدون أي مقدمة أو تمهيد. اكتب النص العربي مباشرة بعد كل علامة. لا تضف شرحاً أو تعليقاً خارج العلامات.";

    var q1="بيانات المشروع:\n"+info+"\n\nاكتب الآن:\n\nVISION:\n[اكتب 4 جمل عن ارتباط المشروع برؤية 2030]\n\nPROGRAMS:\n- [برنامج وطني 1]\n- [برنامج وطني 2]\n- [برنامج وطني 3]\n\nOBJECTIVES:\n- [هدف 1]\n- [هدف 2]\n- [هدف 3]\n- [هدف 4]\n- [هدف 5]\n\nOBJ_D1:\n[شرح الهدف الأول]\n\nOBJ_D2:\n[شرح الهدف الثاني]\n\nOBJ_D3:\n[شرح الهدف الثالث]\n\nPROBLEMS:\n- [مشكلة 1]\n- [مشكلة 2]\n- [مشكلة 3]\n\nPI1:\n[أثر المشكلة الأولى]\n\nPI2:\n[أثر المشكلة الثانية]\n\nPI3:\n[أثر المشكلة الثالثة]\n\nALIGNMENT:\n[3 جمل عن المواءمة القطاعية]\n\nSITUATION:\n[3 جمل عن الوضع الراهن]\n\nDEMAND:\n[4 جمل عن السعة والطلب]\n\nURGENCY:\n[جملتان عن الحاجة العاجلة]\n\nNEG1:\n[تبعة سلبية 1]\n\nNEG2:\n[تبعة سلبية 2]\n\nNEG3:\n[تبعة سلبية 3]\n\nNEG4:\n[تبعة سلبية 4]\n\nBEN_A:\n[العائد الأمني]\n\nBEN_S:\n[العائد الاجتماعي]\n\nBEN_U:\n[العائد العمراني]\n\nBEN_O:\n[العائد التشغيلي]\n\nCONTRACT:\n[4 جمل عن استراتيجية التعاقد]\n\nPAYMENT:\n[جدول الدفعات المقترح]\n\nPROCS:\n- [وثيقة 1]\n- [وثيقة 2]\n- [وثيقة 3]\n- [وثيقة 4]\n- [وثيقة 5]";

    var q2="بيانات المشروع:\n"+info+"\nمدة التنفيذ: "+dn+" شهر\n\nاكتب الآن:\n\nVE1:\n[مقترح هندسة قيمية 1]\n\nVE2:\n[مقترح هندسة قيمية 2]\n\nVE3:\n[مقترح هندسة قيمية 3]\n\nVE4:\n[مقترح هندسة قيمية 4]\n\nR1:\n[خطر مالي 1]\n\nR1M:\n[معالجة الخطر 1]\n\nR2:\n[خطر مالي 2]\n\nR2M:\n[معالجة الخطر 2]\n\nR3:\n[خطر تقني 1]\n\nR3M:\n[معالجة الخطر 3]\n\nR4:\n[خطر تقني 2]\n\nR4M:\n[معالجة الخطر 4]\n\nR5:\n[خطر تشغيلي]\n\nR5M:\n[معالجة الخطر 5]\n\nR6:\n[خطر خارجي]\n\nR6M:\n[معالجة الخطر 6]";

    var res=await Promise.all([callAI(SYS,q1), callAI(SYS,q2)]);
    item.status="generated";
    item.generatedData={t1:res[0],t2:res[1],cn:cn,sn:sn,tot:tot,opx:opx,grd:grd,dn:dn};
    list[idx]=item;
    await writeDB(list);
    ov(false);
    showReport(item);
    ...
    ov(false);
    if(btn){btn.disabled=false;btn.textContent="✅ موافقة وتوليد التقرير";}
    alert("خطأ: "+e.message);
  }
}
...
async function viewRep(id){
  ov(true,"جاري تحميل التقرير...");
  try{
    var list=await readDB();
    var req=list.find(function(x){return x.id===id;});
    ov(false);
    if(req)showReport(req);
    else alert("لم يتم العثور على التقرير");
  }catch(e){ov(false);alert("خطأ: "+e.message);}
}
...
function showReport(req){
  ...
  ge("mtt").textContent="تقرير: "+req.projectName;
  ge("rTitle").textContent=req.projectName;
  ge("rMeta").textContent="جامعة أم القرى · "+(req.location||"")+" · "+(req.fiscalYear||"");
  ge("repMod").classList.add("on");
  build(p,d);
}
