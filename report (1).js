...
function exportPDF(){
  // Build a dedicated print area outside the modal
  var existing=document.getElementById('printArea');
  if(existing)existing.remove();
  
  var printDiv=document.createElement('div');
  printDiv.id='printArea';
  printDiv.style.cssText='position:absolute;top:0;left:0;width:100%;background:#fff;z-index:-1';
  
  slides.forEach(function(s,i){
    var d=document.createElement('div');
    d.className='slide on';
    d.innerHTML=s.html;
    printDiv.appendChild(d);
  });
...
function exportWord(){
  ...
  try{
    ...
    var wordHTML='<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8">'+
      '<title>'+projName+'</title>'+
      '<style>'+
      '@import url("https://fonts.googleapis.com/css2?family=Cairo:wght@400;700;900&display=swap");'+
      'body{font-family:Cairo,Arial,sans-serif;direction:rtl;margin:2cm 2.5cm;font-size:11pt;color:#1a1a1a;line-height:1.8}'+
      'h1{font-size:18pt;color:#0d5c5c;border-bottom:3px solid #c9a227;padding-bottom:8px;margin-bottom:6px}'+
      '.subtitle{font-size:11pt;color:#666;margin-bottom:30px}'+
      'h2{font-size:13pt;color:#0d5c5c;margin-top:28px;margin-bottom:8px;border-right:5px solid #c9a227;padding-right:10px;background:#f0f8f8;padding:6px 10px 6px 0}'+
      'h4{font-size:11pt;color:#085f5f;border-right:3px solid #c9a227;padding-right:8px;margin:16px 0 8px}'+
      'table{width:100%;border-collapse:collapse;margin:10px 0;font-size:10pt}'+
      'th{background:#0d5c5c;color:white;padding:8px 11px;text-align:right;font-weight:700}'+
      'td{padding:7px 11px;border:1px solid #ddd;vertical-align:top;line-height:1.6}'+
      'tr:nth-child(even) td{background:#f5fafa}'+
      'ul{margin:8px 0;padding-right:20px;list-style:none}'+
      'li{margin:5px 0;padding:5px 10px;background:#f5fafa;border-right:2px solid #0d7a7a}'+
      'li::before{content:"◈ ";color:#0d7a7a}'+
      '.hbox{background:#f0f8f8;border:1px solid #c9a22733;border-right:4px solid #0d5c5c;padding:10px 14px;margin:8px 0;font-size:10.5pt}'+
      '.icard{display:inline-block;width:45%;vertical-align:top;border:1px solid #dde;padding:8px 10px;margin:4px;background:#fafcfc}'+
      '.ct{font-size:9pt;color:#0d5c5c;font-weight:700;margin-bottom:4px}'+
      '.page-break{page-break-after:always;border:none;margin:0;padding:0}'+
      '.cover{background:#0d5c5c;color:white;padding:40px;text-align:center;margin-bottom:20px}'+
      '.cover h1{color:white;border-color:rgba(201,162,39,.5)}'+
      '</style></head><body>'+
      '<div class="cover">'+
      '<h1 style="color:#c9a227;border:none;font-size:20pt">تقرير الدراسات الخمسة</h1>'+
      '<div style="font-size:16pt;font-weight:900;margin:16px 0">'+projName+'</div>'+
      '<div style="font-size:11pt;opacity:.7">'+projMeta+'</div>'+
      '</div>';
    
    rows.forEach(function(r,i){
      var clean=r.content
        .replace(/class="hbox gld"/g,'class="hbox"')
        .replace(/class="rbad rh"/g,'style="color:#c03030;font-weight:bold"')
        .replace(/class="rbad rm"/g,'style="color:#c07020;font-weight:bold"')
        .replace(/class="rbad rl"/g,'style="color:#1a7040;font-weight:bold"')
        .replace(/class="recbad"/g,'style="color:#1a7040;font-weight:bold"')
        .replace(/class="tcols"/g,'class="tcols" style=""')
        .replace(/class="[^"]*"/g,'')
        .replace(/style="direction:ltr;/g,'style="');
      wordHTML+='<h2>'+r.title+'</h2>'+clean;
      if((i+1)%3===0) wordHTML+='<hr class="page-break">';
    });
    wordHTML+='</body></html>';
    
    // Use data URI for download - more compatible
    var encoded = 'data:application/vnd.ms-word;charset=utf-8,' + encodeURIComponent(wordHTML);
    var a=document.createElement('a');
    a.href=encoded;
    a.download='تقرير_الدراسات_الخمسة.doc';
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    
    btn.textContent='✅ تم التحميل';
    setTimeout(function(){btn.textContent='📄 تحميل وورد';},3000);
