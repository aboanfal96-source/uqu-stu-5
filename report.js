...
function sec(t,m){
  if(!t)return'—';
  var variants=[m,m.replace(':\n',': '),m.replace(':\n',':')];
  var i=-1,used=m;
  for(var v=0;v<variants.length;v++){i=t.indexOf(variants[v]);if(i!==-1){used=variants[v];break;}}
  if(i===-1)return'—';
  var rest=t.substring(i+used.length);
  var e=rest.search(/\n[A-Z][A-Z_0-9]+[:\s]/);
  return((e===-1?rest:rest.substring(0,e)).trim())||'—';
}
function lns(t,m){
  var raw=sec(t,m);
  if(raw==='—')return['—'];
...
function exportPDF(){
  ...
  var html='';
  slides.forEach(function(s){html+='<div class="sp">'+s.html+'</div>';});
  var win=window.open('','_blank');
  if(!win){alert('يرجى السماح بالنوافذ المنبثقة في المتصفح ثم حاول مجدداً');return;}
  win.document.write('<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8">'+
    ...
    ':root{--teal:#0d7a7a;--gold:#c9a227;--gl:#e8c94a;--r:12px;--tl:#0d7a7a}'+
    '.sp{page-break-after:always;break-after:page;overflow:hidden}'+
    '.sp:last-child{page-break-after:avoid;break-after:avoid}'+
    '.cv{background:linear-gradient(160deg,#0d5c5c,#085050,#052f2f)!important;min-height:100vh;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:50px 40px;position:relative;overflow:hidden}'+
    ...
    '.cv-logo-box img{max-height:70px;width:auto}'+
    '.cv-hdr-div{width:1px;height:72px;background:rgba(201,162,39,.35);margin:0 22px}'+
    '.cv-univ-block{text-align:right;flex:1}'+
    '.cv-univ-ar{font-size:20px;font-weight:900;color:#fff;line-height:1.3;margin-bottom:2px}'+
    ...
    '.cv-yr-line{font-size:13px;color:rgba(255,255,255,.4)}'+
    '.cv-bar{position:absolute;bottom:0;left:0;right:0;height:5px;background:linear-gradient(90deg,#0d7a7a,#c9a227,#0d7a7a)}'+
    '.si{background:linear-gradient(145deg,#085f5f,#063535)!important;min-height:70vh;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px 36px;position:relative}'+
    '.si img{height:44px;width:auto;position:absolute;top:20px;left:28px;opacity:.7}'+
    '.si-n{font-size:68px;font-weight:900;color:rgba(201,162,39,.1);line-height:1;margin-bottom:-6px}'+
    '.si h2{font-size:36px;font-weight:900;color:#e8c94a}'+
    ...
    '.si-kp{background:rgba(255,255,255,.07)!important;border:1px solid rgba(255,255,255,.1);border-radius:7px;padding:6px 12px;font-size:11px;color:rgba(255,255,255,.6)}'+
    ...
    '.cs-hdr h3{font-size:15px;font-weight:800;color:#fff}'+
    '.cs-bdg{background:rgba(201,162,39,.2)!important;border:1px solid rgba(201,162,39,.4);border-radius:5px;padding:3px 10px;font-size:10px;font-weight:700;color:#e8c94a;white-space:nowrap}'+
    ...
    '.cs-body .hbox{background:rgba(13,122,122,.07)!important;border:1px solid rgba(13,122,122,.14);border-radius:9px;padding:12px 14px;margin:8px 0;font-size:12.5px;line-height:1.8;color:#222}'+
    '.cs-body .hbox.gld{background:rgba(201,162,39,.08)!important;border-color:rgba(201,162,39,.2)}'+
    '.cs-body .tcols{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:10px}'+
    '.cs-body .icard{background:rgba(13,122,122,.06)!important;border:1px solid rgba(13,122,122,.12);border-radius:8px;padding:10px 12px}'+
    ...
    '.rh{background:rgba(220,50,50,.1)!important;color:#c03030;border:1px solid rgba(220,50,50,.2)}'+
    '.rm{background:rgba(200,130,0,.1)!important;color:#c07020;border:1px solid rgba(200,130,0,.2)}'+
    '.rl{background:rgba(30,160,80,.1)!important;color:#1a7040;border:1px solid rgba(30,160,80,.25)}'+
    '.recbad{display:inline-flex;align-items:center;gap:3px;background:rgba(30,160,80,.1)!important;color:#1a7040;border:1px solid rgba(30,160,80,.25);border-radius:5px;padding:2px 8px;font-size:10px;font-weight:700}'+
...
function exportWord(){
  ...
  try{
    ...
    var body='<div style="background:#0d5c5c;color:#fff;padding:50px;text-align:center;-webkit-print-color-adjust:exact;print-color-adjust:exact"><div style="font-size:22px;font-weight:900;color:#c9a227;margin-bottom:12px">تقرير الدراسات الخمسة</div><div style="font-size:18px;font-weight:900;margin-bottom:8px">'+projName+'</div><div style="font-size:11px;opacity:.7">'+projMeta+'</div></div>';
    slides.forEach(function(s){
      if(s.t==='cover')return;
      var tmp=document.createElement('div');tmp.innerHTML=s.html;
      if(s.t==='sec'){
        var h2=tmp.querySelector('h2');
        if(h2)body+='<div style="background:#085f5f;color:#c9a227;padding:20px 30px;margin-top:10px;page-break-before:always;-webkit-print-color-adjust:exact;print-color-adjust:exact"><strong style="font-size:18px">'+h2.textContent+'</strong></div>';
        return;
      }
      var hdr=tmp.querySelector('.cs-hdr h3');
      var bdg=tmp.querySelector('.cs-bdg');
      var bd=tmp.querySelector('.cs-body');
      if(bd){
        var clean=bd.innerHTML
          .replace(/class="[^"]*"/g,'')
          .replace(/style="direction:ltr;/g,'style="');
        body+='<div style="margin:10px 0;border:1px solid #dde;border-radius:6px;overflow:hidden">'+
          '<div style="background:#0d7a7a;color:#fff;padding:10px 14px;display:flex;justify-content:space-between;-webkit-print-color-adjust:exact;print-color-adjust:exact">'+
          '<strong style="font-size:13px">'+(hdr?hdr.textContent:'')+'</strong>'+
          '<span style="font-size:10px;opacity:.8">'+(bdg?bdg.textContent:'')+'</span></div>'+
          '<div style="padding:14px;direction:rtl;font-size:10.5pt;line-height:1.8">'+clean+'</div></div>';
      }
    });
    var fullHTML='<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"><title>'+projName+'</title>'+
      '<style>body{font-family:Arial,sans-serif;direction:rtl;margin:1.5cm;font-size:10.5pt;color:#111;line-height:1.8}'+
      ...
      'h4{color:#085f5f;border-right:3px solid #c9a227;padding-right:8px;margin:12px 0 5px;font-size:10.5pt}'+
      '@page{size:A4;margin:1.5cm}</style></head><body>'+body+'</body></html>';
    var win=window.open('','_blank','width=960,height=720');
    if(!win){alert('يرجى السماح بالنوافذ المنبثقة في المتصفح');btn.disabled=false;btn.textContent='📄 تحميل وورد';return;}
    win.document.write(fullHTML);
    win.document.close();
    win.focus();
    btn.textContent='✅ جاهز — اضغط Ctrl+P للطباعة/PDF';
    setTimeout(function(){btn.textContent='📄 تحميل وورد';},6000);
  }catch(e){alert('خطأ في تحضير الوورد: '+e.message);}
  btn.disabled=false;
}